...
export function ScanMoreStrip({ token }: { token: string }) {
  return (
      ...
      <div className="max-w-5xl mx-auto px-6 py-2.5 flex items-center gap-3 flex-wrap">
        ...
        <p className="text-sm text-fg flex-1 min-w-0">
          ...
          <span className="text-fg-dim">
            This report covers only the accounts the author picked.
          </span>
        </p>
 ...
 */
export function ScanMoreRail({ token }: { token: string }) {
  return (
    <aside className="no-print hidden xl:block">
      <div className="sticky top-24 rounded-lg border border-border-1 bg-bg-elev p-4">
        ...
        <p className="text-xs text-fg-dim leading-relaxed mt-2">
          Check the accounts this report left out. Each one comes back with a 0-to-100 score and the
          evidence behind it.
        </p>
...
export function ScanMoreFooter({ token }: { token: string }) {
  return (
    <section className="no-print rounded-lg border border-accent/30 bg-accent/[0.05] p-6 md:p-7">
      ...
      <p className="text-sm text-fg-dim leading-relaxed max-w-xl mb-5">
        This report covers only the accounts its author chose to check. Sign up and this
        investigation is saved to your account, so you can pick the rest of the comment section and
        see who else is real.
      </p>
