                ...
                for y in range(x + 1, len(distinct)):
                    ...
                    if prev is None or weight > prev.weight:
                        best[pair] = Edge(
                            ...
                            # This p-value IS P(evidence | independent), so it becomes the
                            # denominator of the likelihood ratio directly. See probability.py.
                            measured_p=p_adj,
                        )
    return sorted(best.values(), key=lambda e: (e.a, e.b))
...
_SINGLE_WORD = re.compile(r"L\d")


def _is_not_a_template(skeleton: str) -> bool:
    """Shapes that are not templates at all, and must never link two accounts.

    Two of them:

    * **The auto-append shape** (``L4####``): one word then digits. The constitution is explicit
      that digits appended to a handle are generated by the platform on a signup collision and are
      NEVER a tell. Forget this and the signal fires on half of any platform.

    * **A bare single word** (``L9``): no separator, no digits, no structure. This one is subtler
      and it bit in testing. Letter runs are capped at 9 in the skeleton, so ``marchingfern``,
      ``quietwaterbird`` and ``brightpennylane`` all reduce to ``L9`` and would be reported as
      sharing a handle template. They share nothing but being long words. A template is a shape a
      generator produced, and a shape needs more than one part.
    """
    return bool(re.fullmatch(r"L\d#+", skeleton) or _SINGLE_WORD.fullmatch(skeleton))
...
def handle_template(cohort: Cohort) -> list[Edge]:
    ...
    for acc in cohort.accounts:
        ...
        if not sk or _is_not_a_template(sk):
            continue
