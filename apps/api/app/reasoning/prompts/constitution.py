...
_SCORE_DISCIPLINE = ConstitutionBlock(
    ...
    "  (b) a posting rhythm a person does not produce. Do not eyeball this, WORK IT OUT from the "
    "created_at column of that account's own posts: list the gaps between consecutive posts and look "
    "for either gaps that keep landing on nearly the same value, or activity covering all 24 hours "
    "with no multi-hour quiet period on any day. State the figure you found, for example 'a post "
    "roughly every 62 minutes across 30 posts' or 'posts in all 24 hours with no gap longer than 40 "
    "minutes'. Bursts on a busy day and quiet stretches afterwards are how people read the news, and "
    "posting at similar times each day is a routine; neither is this tell. Fewer than about ten "
    "timestamps cannot show a rhythm at all.\n"
...
_SIGNAL_DIMENSIONS = ConstitutionBlock(
    ...
    "- TEMPORAL. The created_at column of this account's own posts, and nothing else. Compute the "
    "gaps between consecutive posts. Two things are elevated: gaps that keep landing on nearly the "
    "same value (a person's gaps scatter, a scheduler's do not), and activity in all 24 hours with no "
    "multi-hour quiet period on any day, because people sleep. Bursts on a busy day then quiet "
    "stretches are human and score low, and so is posting at similar times each day, which is a "
    "routine. Null below about ten timestamped posts: a handful of gaps is not a rhythm.\n"
    ...
    "- AI_WRITING. Deliberately almost always null. It scores above zero ONLY when the account's own "
    ...
    "figure, such as following thousands while almost nobody follows back. A small account, a plain "
    "handle, digits in a handle (platforms append those automatically) and an empty bio are ordinary "
    "and cap this dimension low on their own.\n"
