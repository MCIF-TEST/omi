...
def _render_output_contract(schema: dict) -> str:
    ...
    if COMPREHENSIVE_COMMENTER_ASSESSMENTS_KEY in props:
        commenter_clause = (
            ...
            f"what it has posted — quoting at least two of ITS OWN concrete facts, specific enough that the "
            f"sentence could not be pasted under any other account. Every fact must pass the POINT-TO-THE-"
            f"CELL test: it appears in the account's rows; never state a detail (verification, bio, "
            f"location, off-platform behavior) the evidence does not carry. Mention another account only "
...
COMPREHENSIVE_INVESTIGATION_SYSTEM_TASK = (
    ...
    "2. COLLAPSE AUDIT — scan the per-account scores and reasons: three or more accounts on one number, "
    "or any two reasons that could be swapped without becoming false, means the batch was shortcut — "
    "redo those accounts' Dossier Loops from their own rows. IDENTICAL SCORES ARE A RED FLAG: a real "
    "batch almost always produces a SPREAD, and two accounts may share a number ONLY when their "
    "extracted facts are genuinely equivalent.\n"
    ...
    "these accounts beyond the rows supplied — no verification status, no bio, no location, no off-"
    "platform behavior. Delete any claim without a cell; verify every citation is an alias that exists "
    "in the legend.\n"
