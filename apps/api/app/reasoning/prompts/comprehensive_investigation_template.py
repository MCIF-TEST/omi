...
def _render_output_contract(schema: dict) -> str:
    ...
    return (
        ...
        f"'evidence_refs' citing only evidence ids/aliases present in the evidence. FILL "
        f"evidence_against: it holds the cited reasons these accounts look like real people (the "
        f"multi-year histories, the balanced ratios, the overnight quiet periods, the individually "
        f"written posts), and on a section that comes back clean it is the substance of the report. "
        f"Leaving it empty renders a blank panel to the customer and discards the finding. It is "
        f"empty ONLY when the evidence genuinely holds nothing exculpatory, which is rare, and then "
        f"confidence_rationale must say so explicitly.\n"
        ...
        f"FINAL PASS OVER THE JSON YOU HAVE WRITTEN. Not a restatement of the method above: this is a "
        f"checklist run against the finished object, and it is the last thing you do:\n"
        f"  1. COUNT. One commenter_assessments item per account alias in the legend. Not fewer. "
        f"Compare your count to the number the evidence names.\n"
