...
|------|--------|------|--------------|-------------|
| 2026-06-18 | _(this change)_ | **Behavioral V2 data audit** — proved V1 is a username-morphology shortcut: username=70.9% of gain, coordination=0%; username-only model matches full (AUC 0.985); **no-username collapses to base rate (AUC 0.546, recall 0)**. Verdict: current datasets lack behavioral signal → prioritize engine-feature extraction + controls + analyst labels. Offline only | `ml/evaluation/behavioral_v2_audit/**` | Evidence that V2 needs new data, not more modeling |
| 2026-06-18 | `aaf80fd` | **Omi Behavioral Model V1** — first trainable model: offline XGBoost authenticity baseline (held-out F1 0.876 / AUC 0.982) but ~89% gain = username morphology → baseline only; no `apps/` change, `use_ml_scorer` OFF | `ml/training/behavioral_v1/**`, `ml/evaluation/behavioral_v1/**`, `ml/models/omi-behavioral-v1/**` | Working offline train/eval loop + metric floor |
