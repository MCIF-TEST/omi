...
|------|--------|------|--------------|-------------|
| 2026-06-18 | _(this change)_ | **Omi Neural Network V1 architecture plan** — CPU-only, <$50/mo, glass-box (GBT/monotonic-MLP) learned prior that augments the corroboration-gated rule engine via the dormant `app/ml/scorer.py` seam; docs only | `ml/OMI_NEURAL_NETWORK_V1.md` | Concrete, constraint-faithful plan for a learned scorer; no model trained, scoring untouched |
