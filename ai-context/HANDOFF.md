...
|------|--------|------|--------------|-------------|
| 2026-06-18 | _(this change)_ | **HF dataset organization V1** — inventory + classification (TRAIN/EVAL/ARCHIVE/QUARANTINE) + migration map + HF upload plan/target tree, **documentation only** (no files moved; manifest/prod/tests untouched, per decision — a physical move would break prod path reads + poison-gating) | `datasets/{DATASET_INVENTORY,DATASET_MIGRATION_MAP,HUGGING_FACE_UPLOAD_PLAN}.md` | HF-ready dataset organization on paper; safe, reversible follow-up recipe documented |
| 2026-06-18 | `3fdc19b` | **Behavioral V2 data audit** — proved V1 is a username-morphology shortcut (username=70.9% gain; no-username AUC 0.546≈random); verdict: prioritize new data, not modeling. Offline only | `ml/evaluation/behavioral_v2_audit/**` | Evidence V2 needs new data |
| 2026-06-18 | `aaf80fd` | **Omi Behavioral Model V1** — offline XGBoost authenticity baseline (held-out F1 0.876 / AUC 0.982) but ~89% gain = username morphology → baseline only; no `apps/` change | `ml/training/behavioral_v1/**`, `ml/evaluation/behavioral_v1/**`, `ml/models/omi-behavioral-v1/**` | Working offline train/eval loop |
| 2026-06-18 | `6e5c485` | **Omi Label Schema V1** — ground-truth source audit + engine-independent label contract; PARTIALLY enough labels; docs only | `ml/schemas/OMI_LABEL_SCHEMA_V1.md` | Label sources catalogued |
| 2026-06-18 | `4c0d84e` | **Omi Feature Schema V1** — inventory + canonical 42-dim `build_feature_vector` contract; docs only | `ml/features/OMI_FEATURE_SCHEMA_V1.md` | Proprietary signals catalogued; training-input contract |
