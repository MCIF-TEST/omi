...
|------|--------|------|--------------|-------------|
| 2026-06-18 | _(this change)_ | **Omi Behavioral Model V1** — first trainable model: offline XGBoost (CPU) authenticity baseline in `ml/` (builder+label-map+features+train+eval+card+shadow-plan). Held-out F1 0.876 / AUC 0.982 / FPR 0.043, **but ~89% gain = username morphology** → baseline only, not promotable. No `apps/` change; `use_ml_scorer` OFF | `ml/training/behavioral_v1/**`, `ml/evaluation/behavioral_v1/**`, `ml/models/omi-behavioral-v1/**` | Working offline train/eval loop + metric floor; production untouched |
| 2026-06-18 | `6e5c485` | **Omi Label Schema V1** — audit of every ground-truth source + engine-independent label contract; verdict PARTIALLY enough labels; docs only | `ml/schemas/OMI_LABEL_SCHEMA_V1.md` | Label sources catalogued; leakage rules + contract |
